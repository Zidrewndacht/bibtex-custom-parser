...
def averaged_relevance(papers: dict[str, dict[int, dict]], paper_id: str) -> float | None:
...
def bin_relevance_score(relevance: int | None) -> str:
...
def category_summaries(results: dict,
                       categories: list[tuple[str, list[str]]],
                       field_labels: dict[str, str] | None = None) -> list[dict]:
...
def print_summary(results: dict,
    ...
    print("   Testing: Does lower relevance correlate with "
          "higher uncertainty/contradiction?")
    print("-" * 90)
...
def generate_latex_macros(results: dict, categories: list[tuple[str, list[str]]], output_path: str):
    """Generates a .tex file with \newcommand definitions for key statistics."""
    import pandas as pd
    
    meta = results.get('_meta', {})
    all_papers = results.get('all_papers', {})
    on_topic = results.get('on_topic_only', {})
    off_topic = results.get('off_topic_only', {})
    
    lines = [
        "% AUTO-GENERATED FILE: DO NOT EDIT MANUALLY",
        "% Generated by agreement_core.py",
        f"\\newcommand{{\\StatTotalPapers}}{{{int(meta.get('total_papers', 0)):,}}}",
        f"\\newcommand{{\\StatOnTopicPapers}}{{{len(meta.get('on_topic_ids', [])):,}}}",
        f"\\newcommand{{\\StatOffTopicPapers}}{{{len(meta.get('off_topic_ids', [])):,}}}",
        f"\\newcommand{{\\StatUndeterminedPapers}}{{{len(meta.get('undetermined_ids', [])):,}}}",
        f"\\newcommand{{\\StatTotalFields}}{{{int(meta.get('n_fields', 0))}}}",
    ]
    
    def add_stratum_macros(prefix, s):
        n_obs = int(s.get('n_observations', 0))
        lines.append(f"\\newcommand{{\\Stat{prefix}Obs}}{{{n_obs:,}}}")
        
        p_pct = float(s.get('overall_perfect_pct', 0))
        p_cnt = int(s.get('overall_perfect', 0))
        p_ci_l = float(s.get('overall_perfect_ci_lower', 0))
        p_ci_u = float(s.get('overall_perfect_ci_upper', 0))
        lines.append(f"\\newcommand{{\\Stat{prefix}PerfectPct}}{{{p_pct:.2f}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}PerfectCount}}{{{p_cnt:,}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}PerfectCIL}}{{{p_ci_l:.2f}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}PerfectCIU}}{{{p_ci_u:.2f}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}PerfectFmt}}{{{p_pct:.2f}\\% ({p_cnt:,})}}")
        
        u_pct = float(s.get('overall_uncertain_pct', 0))
        u_cnt = int(s.get('overall_uncertain', 0))
        lines.append(f"\\newcommand{{\\Stat{prefix}UncertainPct}}{{{u_pct:.2f}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}UncertainCount}}{{{u_cnt:,}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}UncertainFmt}}{{{u_pct:.2f}\\% ({u_cnt:,})}}")
        
        c_pct = float(s.get('overall_contradiction_pct', 0))
        c_cnt = int(s.get('overall_contradiction', 0))
        c_ci_l = float(s.get('overall_contradiction_ci_lower', 0))
        c_ci_u = float(s.get('overall_contradiction_ci_upper', 0))
        lines.append(f"\\newcommand{{\\Stat{prefix}ContraPct}}{{{c_pct:.2f}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}ContraCount}}{{{c_cnt:,}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}ContraCIL}}{{{c_ci_l:.2f}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}ContraCIU}}{{{c_ci_u:.2f}}}")
        lines.append(f"\\newcommand{{\\Stat{prefix}ContraFmt}}{{{c_pct:.2f}\\% ({c_cnt:,})}}")

    add_stratum_macros("All", all_papers)
    add_stratum_macros("OnTopic", on_topic)
    add_stratum_macros("OffTopic", off_topic)
    
    n_obs_ot = int(on_topic.get('n_observations', 1)) or 1 # prevent div by zero
    cby = int(on_topic.get('overall_contradiction_biased_yes', 0))
    cbn = int(on_topic.get('overall_contradiction_biased_no', 0))
    cch = int(on_topic.get('overall_contradiction_chaotic', 0))
    
    lines.append(f"\\newcommand{{\\StatOnTopicContraBiasedYesCount}}{{{cby:,}}}")
    lines.append(f"\\newcommand{{\\StatOnTopicContraBiasedYesPct}}{{{(cby/n_obs_ot*100):.2f}}}")
    lines.append(f"\\newcommand{{\\StatOnTopicContraBiasedNoCount}}{{{cbn:,}}}")
    lines.append(f"\\newcommand{{\\StatOnTopicContraBiasedNoPct}}{{{(cbn/n_obs_ot*100):.2f}}}")
    lines.append(f"\\newcommand{{\\StatOnTopicContraChaoticCount}}{{{cch:,}}}")
    lines.append(f"\\newcommand{{\\StatOnTopicContraChaoticPct}}{{{(cch/n_obs_ot*100):.2f}}}")

    if not on_topic.get('field_results', pd.DataFrame()).empty:
        for cat_name, cat_fields in categories:
            cat_df = on_topic['field_results'][on_topic['field_results']['field'].isin(cat_fields)]
            if not cat_df.empty:
                most_contra = cat_df.loc[cat_df['contradiction_pct'].idxmax()]
                safe_cat = cat_name.replace(' ', '').replace('-', '')
                
                lines.append(f"\\newcommand{{\\StatCat{safe_cat}MostContraFieldRaw}}{{{most_contra['field']}}}")
                lines.append(f"\\newcommand{{\\StatCat{safe_cat}MostContraPct}}{{{float(most_contra['contradiction_pct']):.2f}}}")
                lines.append(f"\\newcommand{{\\StatCat{safe_cat}MostContraCount}}{{{int(most_contra['contradiction']):,}}}")
