...
// --- Safe State Getters ---
const _counts = () => window.latestCounts || (typeof latestCounts !== 'undefined' ? latestCounts : {});
const _yearly = () => window.latestYearlyData || (typeof latestYearlyData !== 'undefined' ? latestYearlyData : {});
const _pie = () => window.showPieCharts || (typeof showPieCharts !== 'undefined' ? showPieCharts : false);
const _stacked = () => window.isStacked || (typeof isStacked !== 'undefined' ? isStacked : false);
const _cumulative = () => window.isCumulative || (typeof isCumulative !== 'undefined' ? isCumulative : false);

const _calcCumulative = (arr) => {
    if (typeof calculateCumulativeData === 'function') return calculateCumulativeData(arr);
    if (window.calculateCumulativeData) return window.calculateCumulativeData(arr);
    if (!arr) return []; let sum = 0; return arr.map(v => sum += v);
};
const _getDPR = () => {
    if (typeof getChartDPR === 'function') return getChartDPR();
    if (window.getChartDPR) return window.getChartDPR();
    const dpr = window.devicePixelRatio || 1; return (dpr > 1.0 && dpr < 2.0) ? dpr * 2 : dpr;
};
const _cumLegend = function(chart) {
    if (typeof cumulativeLegendLabels === 'function') return cumulativeLegendLabels.call(this, chart);
    if (window.cumulativeLegendLabels) return window.cumulativeLegendLabels.call(this, chart);
    return Chart.defaults.plugins.legend.labels.generateLabels.call(this, chart);
};
const _renderBarOrPie = (ctx, data, label, type) => {
    if (typeof renderBarOrPieChart === 'function') return renderBarOrPieChart(ctx, data, label, type);
    if (window.renderBarOrPieChart) return window.renderBarOrPieChart(ctx, data, label, type);
    return null;
};
const _reorderStacking = () => {
    if (typeof reorderDatasetsForStacking === 'function') return reorderDatasetsForStacking();
    if (window.reorderDatasetsForStacking) return window.reorderDatasetsForStacking();
};
...
function collectDomainYearlyData(visibleRows) {
    ...
    const yearIdx = (typeof yearCellIndex !== 'undefined' ? yearCellIndex : (window.yearCellIndex || 2));
    
    visibleRows.forEach(row => {
        const yearCell = row.cells[yearIdx]; 
    ...
    if (yData && typeof yData === 'object') {
        yData.techniques = yearlyTechniques;
        yData.features = yearlyFeatures;
    }
}
...
// FIX: Added SMT vs THT Data Preparation
...
function renderDomainLineCharts() {
    const yData = _yearly();
    const isStacked = _stacked();
    const isCumulative = _cumulative();

    // 1. Techniques per Year
    const techniquesYearlyData = yData.techniques || {};
    ...
    const techniqueLineDatasets = TECHNIQUE_FIELDS.map(field => {
        ...
        if (isCumulative) data = _calcCumulative(data);
    ...
    if (window.techniquesPerYearLineChartInstance) { window.techniquesPerYearLineChartInstance.destroy(); window.techniquesPerYearLineChartInstance = null; }
    if (techniquesPerYearCtx && techniqueLineDatasets.length > 0) {
        window.techniquesPerYearLineChartInstance = new Chart(techniquesPerYearCtx, {
            ...
            options: {
                responsive: true, maintainAspectRatio: false, devicePixelRatio: _getDPR(),
                plugins: {
                    legend: { position: 'top', labels: { usePointStyle: true, pointStyle: 'circle', generateLabels: _cumLegend } },
    ...
    // 2. Features per Year
    const featuresYearlyData = yData.features || {};
    ...
    if (window.featuresPerYearLineChartInstance) { window.featuresPerYearLineChartInstance.destroy(); window.featuresPerYearLineChartInstance = null; }
    ...
    if (isStacked) _reorderStacking();
}

function renderDomainCharts() {
    const techniquesCtx = document.getElementById('techniquesPieChart')?.getContext('2d');
    if (window.techniquesBarChartInstance) { window.techniquesBarChartInstance.destroy(); window.techniquesBarChartInstance = null; }
    if (techniquesCtx) {
        window.techniquesBarChartInstance = _renderBarOrPie(techniquesCtx, prepareTechniquesData(), 'Techniques Count', _pie() ? 'pie' : 'bar');
    }

    const featuresCtx = document.getElementById('featuresPieChart')?.getContext('2d');
    if (window.featuresBarChartInstance) { window.featuresBarChartInstance.destroy(); window.featuresBarChartInstance = null; }
    if (featuresCtx) {
        window.featuresBarChartInstance = _renderBarOrPie(featuresCtx, prepareFeaturesData(), 'Features Count', _pie() ? 'pie' : 'bar');
    }
    
    // FIX: Added SMT vs THT Chart Rendering
    const smtVsThtCtx = document.getElementById('SMTvsTHTPieChart')?.getContext('2d');
    if (window.smtVsThtDistChartInstance) { window.smtVsThtDistChartInstance.destroy(); window.smtVsThtDistChartInstance = null; }
    if (smtVsThtCtx) {
        window.smtVsThtDistChartInstance = _renderBarOrPie(smtVsThtCtx, prepareSMTvsTHTData(), 'SMT vs THT', _pie() ? 'pie' : 'bar');
    }
    ...
    if (featureGroupToggle) {
        featureGroupToggle.addEventListener('change', function () {
            if (window.featuresBarChartInstance) {
                ...
                if (ctx) window.featuresBarChartInstance = _renderBarOrPie(ctx, prepareFeaturesData(), 'Features Count', _pie() ? 'pie' : 'bar');
            }
        });
    }
});