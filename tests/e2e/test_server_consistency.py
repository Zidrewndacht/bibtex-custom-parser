...
"""
Every UI mutation is verified against the REAL SQLite database (db_reader),
not only the browser DOM — the DOM is partially cached/mapped client-side.
"""
...
class TestCellCyclingServerSide:
    def test_status_cycle_writes_db_log_and_resets_verification(
            self, page, db_reader):
        cell = page.locator(
            "tr[data-paper-id='p1'] [data-field='is_test_bool'] .emoji-content")
...
class TestFormSavesServerSide:
    def test_trace_only_save_keeps_existing_conflicts(self, page, db_reader):
        """Regression guard: saving ONLY user_trace must not wipe certainty
        for untouched fields (db.py FIX 1)."""
        page.locator(
            "tr[data-paper-id='p2'] .toggle-btn:not(.history-btn)").click()
        ...
        assert paper["main_certainty"]["is_test_bool"] == "conflict", \
            "certainty map was wiped by a comment-only save"
...
class TestPdfUploadServerSide:
    def test_upload_writes_file_and_db(self, page, db_reader, tmp_path):
        ...
        # cleanup so later tests see pristine seed state on disk
        os.unlink(stored)