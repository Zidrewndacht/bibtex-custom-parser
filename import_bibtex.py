...
def import_bibtex(bib_file, db_path):
    ...
    if not cursor.fetchone():
        print("Error: The 'papers' table is missing in the database. Please ensure the v1.2 DB is correctly initialized.")
        conn.close()
        sys.exit(1)

    # Complete v1.2 Schema Columns
    schema_cols = [
    ...
    for entry in bib_db.entries:
        ...
        data['llm_log'] = '[]'
        data['main_certainty'] = '{}'
        data['features'] = json.dumps(globals.DEFAULT_FEATURES)
        data['technique'] = json.dumps(globals.DEFAULT_TECHNIQUE)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Import BibTeX/CSV into existing v1.2 SQLite database for PCB inspection papers')
    parser.add_argument('bib_file', help='Input BibTeX file path')
    parser.add_argument('db_file', help='Target SQLite database file path')
